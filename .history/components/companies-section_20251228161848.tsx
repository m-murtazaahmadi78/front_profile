...
export function CompaniesSection({ data: companies }: { data: Company[] }) {
  ...
  // Duplicate companies to ensure smooth marquee
  ...
  const speed = 100; // 100px per second, adjust for faster/slower
  const duration = screenWidth / speed;
    ...
    <section className="py-20 bg-slate-50 dark:bg-slate-900/50 border-y border-slate-200 dark:border-slate-800 overflow-hidden">
      ...
      <div className="relative w-full overflow-hidden">
        <motion.div
          ...
          transition={{
            duration: duration, // dynamic duration based on screen width
            repeat: Infinity,
            ease: "linear",
          }}
        >
